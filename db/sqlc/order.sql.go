...
const deleteOrdersByCategoryID = `-- name: DeleteOrdersByCategoryID :execrows
DELETE FROM orders
WHERE category_id = $1
`

func (q *Queries) DeleteOrdersByCategoryID(ctx context.Context, categoryID int64) (int64, error) {
	result, err := q.db.ExecContext(ctx, deleteOrdersByCategoryID, categoryID)
	if err != nil {
		return 0, err
	}
